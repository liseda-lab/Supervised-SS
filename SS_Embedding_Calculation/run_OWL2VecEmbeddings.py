...
cache_dir='./SS_Embeddings_Calculation/cache'
...
if __name__== '__main__':
    ...
    if n_arguments == 1:
        ...
        for species, name_datasets in annot_files:
            pre_entity_file = "Data/kgsimDatasets/Prots_" + species + "(ALL).txt"
            entities = [line.strip() for line in open(pre_entity_file).readlines()]

            for GO_domain in GO_domains:
                kg_file = "Data/KGs/goKG_" + species + "_" + domain + ".owl"
                for dataset_name in name_datasets:
                    embeddings = run_OWL2Vec(kg_file, sampler, walker, walk_depth, n_walks,Mix_Type, embed_size,window_arg, iteration, negative_arg, min_count_arg, seed_arg,epoch,URI_Doc_arg,
                                                                                                        Lit_Doc_arg, Mix_Doc_arg, cache_dir, ontology_projection,
                                                                                                        pre_entity_file,
                                                                                                        pre_axiom_file,
                                                                                                        pre_annotation_file, pre_train_model)
                    path_output = 'SS_Embedding_Calculation/Embeddings/' + dataset_name + '/' + domain + '/'
                    ensure_dir(path_output)
                    embedding_file_path = path_output + '/Embeddings_' + dataset + '_owl2vec_skip-gram_wl_' + domain + '.txt'
