...
def recursive(obj, kg_domain, ontology_kg):
    for (s, p, o) in ontology_kg.triples((None, rdflib.RDFS.subClassOf, rdflib.term.URIRef(obj))):
        kg_domain.add((s, p, o))
        recursive(s, kg_domain, ontology_kg)
    return kg_domain

def build_KG_domain(ontology_kg, aspect):
    """
    Builds the KG for a semantic aspect.
    :param ontology_kg: graph composed by ontology part;
    :param aspect: url of the sugbraph root;
    :return: KG (composed by ontology) for a semantic aspect.
    """
    kg_domain = rdflib.Graph()
    kg_domain = recursive(aspect, kg_domain, ontology_kg)
    return kg_domain
